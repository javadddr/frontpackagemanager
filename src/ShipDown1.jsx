...
function ShipDown1({shipments,returnedCus,fetchShipments}) {
  ...
  useEffect(() => {
    if (shipments && backendShipments1) {
...
const [shipmentToDelete, setShipmentToDelete] = useState(null);
  ...
  useEffect(() => {
    fetchShipments();
    fetchBackendShipments1();
  }, []);
  useEffect(() => {
    // Filter shipments based on selected statuses
    if (returnedCus) {
      const filtered = returnedCus.filter(shipment => selected.includes(shipment.delivery_status));
      setFilteredShipments(filtered);
    }
  }, [returnedCus, selected]);
...
const handleEdit = (id) => {
  ...
  >
        ...
        <p className="text-center text-lg text-gray-600">No shipment found. Create your first return or change the filters!</p>
      </div>
        )}
        </div>
