...
import StepTwoCus from './StepTwoCus';
import StepThreeCus from './StepThreeCus';
const CreateShipCus = ({ handleCloseModal,fetchShipments,fetchBackendShipments ,setBackendShipments,setOpen, setProgress,setMessages, fetchBackendShipments1,fetchBackendShipments2,setShipments, setTrackingNumbers, trackingNumbers, couriers, filteredCarriers, setFilteredCarriers }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const shipmentResponse = await fetch(`${backendUrl}/api/shipments1`, {
        method: 'POST',
        headers: {
     ...
     if (selectedProducts.length > 0) {
      ...
      const recipient = selectedCustomerId || selectedVendorId || ""; // Use customer or vendor ID as recipient
    ...
    if (selectedProducts.length > 0) {
      ...
      const updatedShipments = await fetchBackendShipments();
      setBackendShipments(updatedShipments);
